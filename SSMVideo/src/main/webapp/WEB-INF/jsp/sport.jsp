				...
				<c:if test="${fn:length(map['mappingMovies'])>6 }">
					<span>共<input style="width: 30px" type="text"
						...
						type="text" value="${map['count'] } 条"></span>&nbsp;&nbsp;&nbsp;&nbsp;
								<span><a
						href="/showsport?page=${map['nowPage']-1 }&movietypedetail=${map['movietypedetail']}
								&movietypehead=${map['movietypehead']}"><button>上一页</button></a></span>
								${map['bar'] }
								<span><a
						href="/showsport?page=${map['nowPage']+1 }&movietypedetail=${map['movietypedetail']}&movietypehead=${map['movietypehead']}"><button>下一页</button></a></span>
			...
			<c:if test="${fn:length(map['mappingMovies'])<6 }">
				<div
					...
					<span>共<input style="width: 30px" type="text"
						...
						type="text" value="${map['count'] } 条"></span>&nbsp;&nbsp;&nbsp;&nbsp;
					<span><a
						href="/showsport?page=${map['nowPage']-1 }&movietypedetail=${map['movietypedetail']}&movietypehead=${map['movietypehead']}"><button>上一页</button></a></span>
					${map['bar'] } <span><a
						href="/showsport?page=${map['nowPage']+1 }&movietypedetail=${map['movietypedetail']}&movietypehead=${map['movietypehead']}"><button>下一页</button></a></span>
